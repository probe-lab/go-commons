...
func NewRootCommand(cmd *cli.Command) (*RootCommand, *RootCommandConfig) {
	...
	cmd.Flags = append(cmd.Flags, []cli.Flag{
		...
		&cli.StringFlag{
			...
			Sources:     cli.EnvVars(cfg.EnvPrefix + "TRACING_ENDPOINT"),
			Usage:       "OTLP gRPC collector for traces as host:port or URL. Empty falls back to OTEL_EXPORTER_OTLP_ENDPOINT, then localhost:4317.",
		...
		&cli.BoolFlag{
			...
			Sources:     cli.EnvVars(cfg.EnvPrefix + "TRACING_INSECURE"),
			Usage:       "Send traces without TLS. Unset falls back to OTEL_EXPORTER_OTLP_INSECURE.",
		...
		&cli.StringMapFlag{
			...
			Sources:     cli.EnvVars(cfg.EnvPrefix + "TRACING_HEADERS"),
			Usage:       "Headers sent with every trace export as key=value, comma separated. Empty falls back to OTEL_EXPORTER_OTLP_HEADERS.",
		...
		&cli.StringFlag{
			...
			Usage:       "The AWS region that this service runs in.",
			Value:       cfg.AWSRegion,
